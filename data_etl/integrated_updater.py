...
def run_integrated_update(days: int = 1, skip_min: bool = False):
    """
    主程式邏輯，可供外部呼叫。
    
    Args:
        days (int): 更新最近N天的週期K線.
        skip_min (bool): 是否跳過分鐘K線更新.
    """
    start_time = datetime.now()
    logging.info("=" * 60)
    logging.info("整合更新程式開始執行")
    logging.info(f"開始時間: {start_time}")
    logging.info(f"參數: days={days}, skip_min={skip_min}")
    logging.info("=" * 60)
    ...
    # 步驟 2: 更新週期K線
    if not run_cycle_updater(days=days):
        success = False
...
def main_cli():
    ...
    if len(sys.argv) > 1:
        for arg in sys.argv[1:]:
            if arg.startswith('--days='):
            ...
            elif arg == '--skip-futures' or arg == '--skip-min': # 保持相容
                skip_futures = True
            elif arg == '--help':
                print("使用方式:")
                ...
                print("  --skip-min        跳過期貨分鐘K線更新，只更新週期K線")
                print("  --help            顯示此說明")
                return 0
    
    return run_integrated_update(days=update_days, skip_min=skip_futures)
